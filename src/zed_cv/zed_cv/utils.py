...
#lower_yellow = np.array([20, 75, 100]) #default Yellow only
#lower_yellow = np.array([0, 50, 110]) #orange and yellow
#upper_yellow = np.array([40, 255, 255]) #default also detects white barrier
#upper_yellow = np.array([40, 230, 255])

# look ahead pixel (y coordinate) (range 0-270)
# increasing this decreases the look ahead distance
# val = 200
val = 250 # car hight position in raw image

def find_lane(bev,lower_yellow,upper_yellow):
    num_roi_thres=10
    ...
    ############### HANDLING EDGE CASES ##############
    ## len(left_lane_x) < threshold threshold was 10

    # WHEN ONLY RIGHT LANE IS DETECTED
    if len(left_lane_x) <num_roi_thres and len(right_lane_x) > num_roi_thres:
        ...
        # when only 1 line detected put the cenre_fit_x to right_fit_x?

        if np.array_equal(lower_yellow,np.array([0, 50, 110])):
            centre_fit_x = right_fit_x
        #elif lower_yellow == np.array([20, 75, 100]):
        else:
                
            centre_fit_x = (right_fit_x - 200)
           
        
        # defalt: no if elif, centre_fit_x = (right_fit_x - 200)
        #centre_fit_x = right_fit_x
        ...
        error = error = ((centre_fit_x[val]-240)) #based on the fact that 240 is the middle of the image width
        return img_with_points,error
    
    
    # WHEN ONLY LEFT LANE IS DETECTED
    elif len(left_lane_x) > num_roi_thres and len(right_lane_x) < num_roi_thres:
        ...
        #error = error = ((240-centre_fit_x[val]))
        return img_with_points,error
